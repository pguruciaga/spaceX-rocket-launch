...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        #data['class'] = data['class'].map({1: 'Success', 0: 'Failure'})
        fig = px.pie(data, values='count', 
            names='class', 
            title=f'Success and failure rate for site {entered_site}')
    return fig
...
def get_scatter_chart(entered_site, payload_range):
    ...
    if entered_site == 'ALL':
        ...
        filtered_df = filtered_df[filtered_df['Launch Site'] == entered_site] # Also filter by site
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category', 
